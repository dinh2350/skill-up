...
function visualizeSumFormula(nums) {
    ...
    console.log(`  Expected - Actual = ${expectedSum} - ${actualSum} = ${missing}`);
    console.log('');
...
function visualizeXOR(nums) {
    ...
    for (let i = 0; i < n; i++) {
        const before = result;
        result ^= i;
        console.log(`Step ${i * 2 + 1}:       result = ${before} ^ ${i} (index) = ${result}`);
        
        const beforeValue = result;
        result ^= nums[i];
        console.log(`Step ${i * 2 + 2}:       result = ${beforeValue} ^ ${nums[i]} (value) = ${result}`);
    }
...
function comparePerformance() {
    ...
    sizes.forEach(size => {
        ...
        console.log(`Array Size: ${size.toLocaleString()} elements (missing: ${missingIdx})`);
...
function runTests() {
    ...
    const testCases = [
        { input: [3, 0, 1], expected: 2, description: 'Example 1: Unsorted small array' },
        { input: [0, 1], expected: 2, description: 'Example 2: Missing last number' },
        { input: [9, 6, 4, 2, 3, 5, 7, 0, 1], expected: 8, description: 'Example 3: Large unsorted' },
        { input: [0], expected: 1, description: 'Single element, missing 1' },
        { input: [1], expected: 0, description: 'Single element, missing 0' },
        { input: [0, 1, 2, 3, 4, 5, 7], expected: 6, description: 'Missing middle in sorted' },
...
console.log('  2. XOR: result = n; XOR all i and nums[i]     [Shows advanced knowledge]');
